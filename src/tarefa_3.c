...
bool tarefa_3(bool p, bool q, bool r) {
...
int main() {
    ...
    bool answer[CONSEQUENCE_SIZE] = {false, false, false, false, false, false, false, true};

    bool alright = true;

    printf("Tarefa 3: (P && Q) && R\n");
    for(int i = 0; i < PREMISES_SIZE; i++) {
        for(int j = 0; j < PREMISES_SIZE; j++) {
            for(int k = 0; k < PREMISES_SIZE; k++) {
                if(tarefa_3(ps[i], qs[j], rs[k]) == answer[i * PREMISES_SIZE * PREMISES_SIZE + j * PREMISES_SIZE + k]) {
                    printf("    Correto: ");
                } else {
                    printf("  Incorreto: ");
                    alright = false;
