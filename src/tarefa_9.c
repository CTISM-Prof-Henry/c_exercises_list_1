...
bool tarefa_9(bool p, bool q, bool r) {
...
int main() {
    ...
    bool answer[CONSEQUENCE_SIZE] = {true, true, true, true, true, false, false, false};

    bool alright = true;

    printf("Tarefa 9: ~(P && (Q || R))\n");
    for(int i = 0; i < PREMISES_SIZE; i++) {
        for(int j = 0; j < PREMISES_SIZE; j++) {
            for(int k = 0; k < PREMISES_SIZE; k++) {
                if(tarefa_9(ps[i], qs[j], rs[k]) == answer[i * PREMISES_SIZE * PREMISES_SIZE + j * PREMISES_SIZE + k]) {
                    printf("    Correto: ");
                } else {
                    printf("  Incorreto: ");
                    alright = false;
